...
- **D39 json/xml 구조 변환** (2026-08-11, markup-lane) — 지금은 평문 통과
  (`fileconvert.py TEXT_EXTS={"txt","md","markdown","json","log","xml"}` — 텍스트
  레인, 변환·구조화 없이 그대로 블록화).
  **정정(2026-08-17)**: 원래 근거("markitdown 도 하지 않는 일이라 손실 없음")가
  낡았다 — markitdown 은 Phase 2d(`a8f9818`)에서 **완전히 제거**됐고
  `parse_service/tests/test_no_markitdown.py` 재유입 방지 가드까지 있다(더 이상
  이 코드베이스에 존재하지 않는 도구를 비교 기준으로 삼고 있었다). 동작 자체(평문
  통과)는 여전히 사실이지만, "그래서 손실이 없다"는 정당화는 markitdown 비교가
  아니라 **텍스트 레인이 원래 그런 설계**(변환·파싱 불필요한 포맷은 그대로
  블록화)라는 걸로 다시 서술해야 한다. 정형 API 응답 같은 문서가 실제로 들어오기
  시작하면 키 계층 → 헤딩/표 변환을 검토한다(이 결정 자체는 변경 없음).
