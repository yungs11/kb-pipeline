...
- **더 흔한 실패는 "겹침 0" 이 아니라 "겹침 동점" 이다.** 겹침이 잡히긴 하는데 그게
  변별력 없는 흔한 단어 하나뿐이면 **정렬은 rank 가 한다** — 겹침 0 과 실질이 같다.
  이건 겹침 0 만 세는 지표로는 안 잡힌다.

**실제 코퍼스로 실측(2026-08-09)** — `frontend/public/comparison_report.html` 의 현업
verbatim 질문과 사규 6종에서 만든 리포트 10건. 기본값 `top_k=5`:

| 질문 | 고른 리포트 |
|---|---|
| 장모님 팔순 잔치는 휴가 며칠이야? | `[1,2,4,5,3]` |
| 처제 결혼에 휴가 가능한가? | `[1,2,4,5,3]` |
| 삼촌 환갑에 저는 며칠 휴가 쓸수있나요? | `[1,2,4,5,3]` |
| 이사하는데 휴가 나오나? | `[1,2,4,5,3]` |

**네 질문이 완전히 동일한 5건을 고른다.** 겹침은 모두 1 이고 그 1 은 넷 다 `"휴가"`
한 단어다 — 거의 모든 리포트에 있으니 변별력이 0 이다. 즉 팔순·처제결혼·환갑·이사 중
무엇을 물어도 **같은 근거로 답을 만든다.** 특정 주제 질문 20종 집계: 겹침 0 2건,
겹침 동점 4건, 서로 다른 선정 집합 12/20.

지금 이게 치명적이지 않은 이유는 **`top_k=5` 가 코퍼스의 절반**이라서다(리포트 10건).
라이브는 15건이라 top-5 = 33% 다. **리포트가 늘면 그만큼 나빠진다** — 이 완화는
규모에 의존한다.

> 정정: 앞선 커밋 메시지에 `"주소변경 절차"`(붙여쓰기)가 오답을 고른다고 적었는데,
> 그건 리포트 2건짜리 합성 픽스처에서만 재현된다. 실제 코퍼스(10건)에서는 다른 토큰이
> 받쳐줘 뒤집히지 않았다. 붙여쓰기는 **그 복합어가 유일한 변별 토큰일 때만** 문제가 된다.
...
**결정(2026-08-10): 수용하고 배포한다.** global 의 승인된 용도(넓은 질문)에서는 정상
동작하고, UI 토글 설명이 특정 주제 질문을 일반 검색으로 유도한다. 근본 수정(형태소
분석기·임베딩 기반 선정)은 폐쇄망 사전 반입 또는 리포트 임베딩 저장소 신설이 필요해
범위 밖 → **D38**.

그래서 **게이트 판정을 두 축으로 갈랐다.** 축2(특정 주제) 실패는 배포를 막지 않는다 —
하나로 합치면 정상 배포가 영구 차단되고, 매번 "보류" 를 뱉는 가드는 곧 아무도 안 돌린다
(이 프로젝트에 전례가 있다: verify-bundle 의 import 검사를 안 돌려 fitz 배포 버그를
통과시킨 건). 축2 는 회귀 감시 숫자로 남기고, D38 착수 후 `--strict` 로 승격한다.

착수 트리거: (a) 사용자가 global 에 특정 주제 질문을 던지는 것이 관측 (b) 수치 악화
(c) 리포트 증가로 `top_k` 가 코퍼스의 소수가 될 때.
...
**처분: 보류**(사용자 결정 2026-08-13) → `docs/superpowers/specs/2026-08-13-adopt-vl-table-deferred.md`

- `vl` 레인은 애초에 표 승계가 없다(VL 산출물만 쓴다) — **이미 사용자가 원하는 동작**이라
  지금 당장 손해가 나는 경로가 아니다.
- ⚠️ **"사문이 되어 자동 정리" 될 거라던 가정은 틀렸다.** V7-0(아래 03-dev-progress)에서
  세로형 스캔 2장이 `OCR_NEEDED` 로 남아 `paddle_gw` 레인에 가는 것이 확인됐다 —
  hybrid 도 그 규칙도 **계속 돈다.** 제거하려면 **명시적 결정**이 필요하다.
- 다시 볼 시점: **V7(hybrid 발화 실측) 직후.** 부동산교재 p7·p49 에 표가 있는지, VL 과
  paddle 중 어느 쪽이 나은지가 그때 **처음으로** 관측된다(지금은 표본이 0건이라 판단 불가).
...
## Phase 2b-1 — PageTrace 관측 (2026-08-13~14)

**동작을 바꾸지 않고 기록만 추가**한 phase. 산출물은 `page_traces`(전 페이지)와
로그 두 출구(문서 요약 1줄 + 페이지 표 컬럼).

### 설계가 세 번 뒤집힌 끝에 남은 것

원래 2b 는 **전 레인 5단 폴백 + 관측**이었다. 판단이 이렇게 움직였다:

1. **(내) "VL 이 못 읽는 페이지가 0장이니 체인 근거가 없다"** → **틀린 프레임.**
2. **(사용자) "못 읽는 페이지가 아니라 폐쇄망에서 122b 가 아플 때 9b 로 가야 한다"**
   → 목적은 **능력이 아니라 가용성**. M3(9b 92%)·M4(GW 100%)가 그 증거가 됐다.
3. **(사용자, 최종) "9b 로 가는 것보다 122b 가 문제 있다고 멈추는 게 맞다."**
   → 조용히 내려가면 **결과물이 정상으로 보여** 열화를 못 본다. **폴백을 전부 폐기.**

**같은 실측이 정반대 결론을 지지할 수 있고, 그 선택은 제품 판단이다.**
M3/M4 수치는 "구제 가능함" 의 기록이지 "폴백을 두어야 한다" 의 근거가 아니다.

그래서 2b 는 **2b-1(관측)** 과 **2b-2(실패 정책)** 로 갈렸다 — 2b 통합 검증이
must-fix **6→8→10** 으로 늘었고 **10건 중 5건이 "문서 단위 실패" 하나에서 파생**했다.

### 삼킴 3층 — 관측이 눈멀어 있던 이유

V0 가 "결과만으로는 서빙 이상과 페이지 난이도를 못 가린다" 를 보여줬는데,
정작 **예외가 3층에서 빈 리스트로 삼켜져** 상위가 아무것도 못 보고 있었다:

| 층 | 위치 | 삼킨 것 |
|---|---|---|
| 1 | `ocr_file_to_elements` 페이지 루프 | 페이지별 VL 예외 — **여기서 이미 사라져 2층에 안 간다** |
| 2 | `ocr_elements_many` gather | job 단위 예외 |
| 3 | `_parse_routed` 배치 | 배치 전체 실패 — meta 자체가 없다 |

→ `VLCallMeta(model/tokens/finish/elapsed/error)` 를 신설해 각 층이 사유를 실어 올린다.
**동작은 그대로**(실패 비치명 계약 유지). 반환 계약이 `(elements, metas)` 쌍이 되면서
픽스처 12곳을 함께 고쳤다 — 안 고치면 `meta` 자리에 float 이 들어가 **조용히 이상한 값이 쌓인다**.

### `byte-identical` 은 달성 불가다 — 실측이 수용기준을 바꿨다

같은 2a 코드로 같은 문서를 **두 번** 돌렸더니:

| 문서 | pages | tables | blocks | chars |
|---|--:|--:|--:|--:|
| `m2_3lane_7p` | 7 = 7 ✅ | 7 = 7 ✅ | 126 → 117 ❌ | 5,837 → 5,820 ❌ |
| `odl_vl_14p` | 14 = 14 ✅ | **23 → 24 ❌** | 101 → 88 ❌ | 13,103 → 12,931 ❌ |

`vl` 레인이 VL 비결정성을 그대로 받아 **표 개수까지 흔들린다.** 내용을 비교하면
"회귀" 와 "VL 흔들림" 을 구분할 수 없다 — **구현 전에 알았기에 없는 회귀를 쫓지 않았다.**

→ **판정은 구조로**(사용자 결정): 페이지 수 · 레인 배정 · `lane_dist` · `doc_lane` ·
`narrate_pages` · `diagram_pages`. 기준선은
`_workspace/planA-measurements/baseline-2a/structure.json`.
**착수 후에는 같은 조건을 재현할 수 없어 미리 캡처했다.**

### `page_verdicts` 를 개명하지 않은 이유

`page_traces` 를 **추가**하고 `page_verdicts` 는 그대로 뒀다(사용자 결정).

개명하면 테스트 **13곳이 인덱스 기반**이라 전부 고쳐야 한다 —
`page_verdicts[0]`("게이트 대상 중 첫째") vs `page_traces[0]`("1페이지") 로 **의미가 바뀐다**.
그 과정에서 단언이 조용히 허술해질 수 있다(사유·signals 단언을 빠뜨려도 **통과**하고 개수도 그대로).
**소비처 4곳 · 외부 레포 0건**이라 급하지도 않다 — kb-backend 가 읽는 건 Phase 2.5 다.
제거는 Phase 4.

**결과: 기존 13곳이 한 줄도 수정되지 않고 통과한다 — 그것이 "개명 안 함" 의 증거다.**

### 구현에서 잡힌 결함 — 테스트가 문서 검토보다 나았다

plan 에 "`skip`·quarantine 은 **정상적으로** 비는 경로라 `empty` 와 구분해야 2b-2 의
문서실패 대상집합을 정할 수 있다" 고 적어놓고, 정작 구현에서 **"blocks 비면 empty"** 규칙이
`skip` 까지 덮었다. **문서 검토가 아니라 앵커가 잡았다.**
→ `_EMPTY_IS_NORMAL = {"skip", "unclassified"}`.
...
| V4 | **구조 12항목 동일 — 동작 무변경** |
| V7 | env 신설 0 — 폐쇄망 선언처 동기화가 이 phase 에선 no-op |

실측 출력(3레인 합성 문서):
```
source: gw_hybrid 2p · odl_md 3p · vl 2p
