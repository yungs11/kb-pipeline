...
def run_paddle_gateway(pdf_bytes: bytes, filename: str,
    ...
    def one(rp, probe: bool = False) -> tuple[int, list, list, tuple | None]:
        ...
        return rp.page_number, blocks, layout, page_size
    ...
    # layout/page_size 는 **순수 추가** — 기존 두 키(page_number, blocks)는 그대로다.
    return [{"page_number": n, "blocks": blocks, "layout": layout, "page_size": page_size}
            for n, blocks, layout, page_size in sorted(results, key=lambda t: t[0])]
