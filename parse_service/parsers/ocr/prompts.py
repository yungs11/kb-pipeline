...
# 2026-08-12 재통합: scan-lane 개정판 채택(§5-5). 옛 판은 '순서도다' 를 전제하고 흐름서술만
# 지시해, 차트·표·일반 본문 페이지에서 **없는 흐름을 지어내거나** "다이어그램이 아니므로…"
# 같은 메타 문장을 본문 블록으로 흘렸다. 개정판은 **먼저 무엇인지 판단**하게 하고 4분기
# (순서도/차트/표/그 외)로 가른다 — 그 외는 빈 배열만.
