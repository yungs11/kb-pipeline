...
- 현재 Stage0 게이트(`pipeline.py:452-497`, 원시 바이트 → `deps.docguard.check`)를 **파서 후단 게이트**로 교체:
  - **provider == `kb_pipeline` 이고** 확장자가 xlsx/xlsm 이면(사용자 결정 2026-06-29: 게이트는 kb_pipeline provider 전용. dify/edgequake/raganything/ragflow 는 미적용): excel-parser `/parse` 호출 → `stats.gate_summary` → doc_guard `/v1/check-excel` 호출.
  - fail → 기존과 동일하게 `status="rejected"` + `document_id=rec.document_id` + `_build_gate_popup(report)` + `set_doc_guard_result`(rejected 분기 prepare/create 유지).
  - pass / 비-xlsx / 비-kb_pipeline → 게이트 통과(13규칙 전면 제거). `report=None` 초기화 + 공유 `set_doc_guard_result` 라인 `if report is not None` 가드.
  - ⚠️ kb_pipeline xlsx 통과 시 게이트의 excel-parser 파싱 + tail 의 facade 파싱이 **이중 파싱**(spec §6.3 의 결정 — v1 허용, 후속 최적화).
...
## 9. 테스트 (수용 기준)
> **게이트 단위 = 파일 단위(사용자 결정 2026-06-29).** `gate_summary.ok` = 모든 시트 findings 0건. 한 시트라도 findings 면 파일 차단.

**차단되어야 정상:**
- 외부데이터 시트2(나란히 2표) → side_by_side 차단, 메시지에 `A2,C2`
- WBS(#REF! 포함) → ref_error 차단, 메시지에 해당 셀
- **자산목록(`신한자산신탁_자산목록_v20251013.xlsx`) → 파일 차단** — `NAC연계` 시트가 진짜 side_by_side(업무망/인터넷망 두 표, 빈열 분리). 단 `접근제어 적용 대상` 시트 **자체는 clean**(sheet-level 테스트로 보장).

**통과되어야 정상:**
- 외부데이터 시트1 / aws_cost_estimate → 통과
- 위임전결기준표 → 통과(향후 고도화)

**side_by_side 거짓양성 가드(실측):** `전체자산`(한 표에 HOSTNAME 열 2개)·`접근제어_조사`(사람별 열 매트릭스)는 **차단하면 안 됨**. → side_by_side 는 (A)인덱스열(순번/No 등) 중복 OR (B)≥2라벨 블록 통째 반복 일 때만 발화.
