...
- 이 실측은 §4 테스트가 아니라 **배포 전 게이트**로 남긴다(자동화된 관련성 단언은 임계값
  설계가 필요해 범위를 넘는다).

> **실제 결과(§6.3)**: 위 "보류" 조건이 **특정 주제 질문에서 부분적으로 성립했다**(6/20).
> 그러나 넓은 질문(승인된 용도)에서는 정상이었고, 그 축에서 rank 순 선정은 오히려 맞는
> 동작이다. 그래서 보류 대신 **축2 를 알려진 한계로 수용**하고 배포했다 — 근거와 관측
> 트리거는 §6.3. 게이트는 `scripts/check_global_rank.py` 로 자동화됐다(수동 아님).
...
### 6.3 배포 전 게이트 — 실행 완료, 축2 수용 후 배포 승인
