...
**상태(2026-08-10)** — **수용하고 배포하기로 결정**했다. 축2 실패는 배포를 막지 않고
`check_global_rank.py` 가 회귀 감시 숫자로 남긴다. 착수하면 `--strict` 로 게이트 승격.
...
> 틀렸다.
>
> 1. **`ingest` kind 는 아무도 안 쓴다.** kb 는 `/jobs/parse` → `/jobs/chunk` →
>    `/jobs/insert` **세 개의 별도 잡**으로 부른다(`kb_pipeline_client.py:7` — "v1 의
>    불투명 `/ingest` 한 방 호출 폐기"). facade 에 `/ingest`·`/jobs/ingest` 가 남아
>    있지만 호출부가 없다. 즉 "ingest 취소가 최대 4시간" 은 실제로 일어나지 않는다.
> 2. **단계 경계 취소는 구현된 적이 없다.** `set_stage` 는 `cancel_requested` 를 보지
>    않았고, runner·worker 어디에도 취소를 읽는 코드가 없다.
>
> 그리고 파이프라인이 잡 3개로 갈려 있으므로 **단계 사이는 이미 잡 경계**다 — parse 도중
> 취소하면 kb 가 chunk 를 아예 제출하지 않는다. 남는 낭비는 "진행 중인 그 한 잡이 끝까지
> 도는 것" 하나뿐이다.
>
> **이번에 한 것 (1·2)**
> - **즉시 취소된 잡의 객체를 곧장 지운다.** `repo.cancel` 이 `*_ref` 를 함께 돌려주고
>   API 가 커밋 뒤 삭제한다. 예전에는 TTL GC(기본 72h)까지 남아, 큰 스캔 PDF 를 잘못
>   올려 취소해도 수십 MB 가 사흘을 버텼다.
> - **`stage='inserting'` 이면 취소를 거부한다(409).** edgequake 에 이미 제출한 뒤라
>   여기서 멈추면 **부분 적재**가 남는다 — D5 에서 이 단계를 재시도·회수 대상에서 뺀
>   것과 같은 판정이다. 상태도 `cancel_requested` 플래그도 건드리지 않는다. 소비자는
>   `GET /jobs/{id}` 의 `stage` 로 취소 버튼을 감춘다.
> - `repo.cancel` 반환이 문자열 → dict 로 바뀌었다(인메모리 더블도 같은 계약).
>
> **행은 지우지 않는다.** kb 의 폴링 루프가 `GET /jobs/{id}` 를 계속 두드리는데
> (`kb_pipeline_client.py:186`), 행이 사라지면 404 → `raise_for_status()` → 취소가
> **알 수 없는 오류로** 보인다. 행 삭제는 TTL GC 몫이다.
>
> **남은 것 (3)**: `running` 인 parse·chunk 의 **즉시 중단**. 다운스트림 폴링 루프에
> 취소 훅을 뚫어야 한다(`parse_client.py`·`edgequake.py`). 사용자 판단으로 보류 —
> 잡 경계가 이미 취소 지점을 주므로 실익 대비 범위가 크다.
...
### D16. 같은 MinIO 버킷+프리픽스를 공유하는 두 배포 — ✅ **구현 완료 (2026-08-05)**

> **프리픽스 격리**로 간다(선택지 1). 스윕을 opt-in 으로 끄는 안(2)은 고아가 안 치워져
> D2/D20 을 무력화하고, 객체 태깅(3)은 비용 대비 이득이 낮다.
>
> 고친 것:
> - `KBP_STAGING_PREFIX` 를 compose 에 노출(잡 큐 쪽 `KBP_JOB_MINIO_PREFIX` 는 이미 있었다).
> - **쓰는 쪽과 지우는 쪽을 한 함수로 묶었다** — `staging_gc.staging_prefix()`. 예전에는
>   `objects.py` 가 `parse-staging/` 을 하드코딩하고 스윕만 env 를 봐서, 값을 바꾸면
>   격리는 안 되고 **스윕만 헛도는** 상태가 됐다. 테스트가 두 값의 일치를 고정한다.
> - worker 기동 시 `bucket/jobs/staging` 프리픽스를 로그에 남긴다. 격리가 프리픽스뿐인데
>   어떤 값으로 도는지 안 보이면, 남의 staging 을 지우고 있어도 알 방법이 없다.
> - `docs/airgap-deploy.md` 부록 A(외부 MinIO 재사용)에 **필수 단계**로 명시 — 왜 위험한지
>   (다른 Postgres → "내 행에 없으니 고아" 오판)와 **둘 다** 나눠야 한다는 점까지.
>
> 기본값은 그대로다(`kbp-jobs`·`parse-staging`) — 바꾸면 기존 배포가 적재된 객체를 못 찾는다.
...
리포트를 고른다. 한국어는 교착어라 조사·어미가 붙으면 토큰이 빗나간다.

실측(2026-08-09) — 실제 코퍼스(사규 6종 → 리포트 10건) + 현업 verbatim 질문,
`scripts/check_global_rank.py`:
- **기본값 `top_k=5` 에서 휴가 구어체 4종이 완전히 동일한 리포트 5건을 골랐다.**
  "장모님 팔순" / "처제 결혼" / "삼촌 환갑" / "이사하는데 휴가" → 모두 `[1,2,4,5,3]`.
  겹침은 전부 1 이고 그 1 은 넷 다 `"휴가"` 한 단어다(변별력 0) → 정렬은 rank 가 했다.
- 겹침 0 도 있다: `"위험관리전담부서에서 보고는 어느때에 하는거지?"` → 정답 리포트 #7 이
  선정에 못 들어온다(조사·어미로 토큰 전부 빗나감).
- 집계 20종: 겹침 0 **2건**, 겹침 동점 **4건**, 서로 다른 선정 집합 **12/20**.

**규모 의존 완화**: 지금 파국이 아닌 이유는 `top_k=5` 가 리포트 10건의 절반이라서다.
라이브 15건이면 33%, 더 늘면 더 나빠진다.

**왜 범위 밖** — 고치려면 형태소 분석기(kiwi/mecab) 또는 임베딩 기반 리포트 선정이 필요하다.
전자는 폐쇄망 사전 반입 + 이미지 크기 증가, 후자는 리포트 임베딩 저장소·재계산 경로 신설이다.
둘 다 B(버튼 노출)의 범위를 크게 넘는다. **B 는 기능을 노출하는 작업이고, 노출 자체는
현재 선정 품질로도 의도된 용도(넓은 질문)에서 동작한다** — 넓은 질문은 겹침 0 이 정상이고
`rank` 순 선정이 오히려 맞다.

**지금 무엇으로 막고 있나**
1. UI 토글 설명이 global 을 "넓은 질문" 용도로 안내하고 특정 주제 질문은 local 로 유도한다.
2. `scripts/check_global_rank.py` 가 배포 전에 이 결함을 **드러낸다**(답변만 봐서는 안 보인다).

**언제 필요해지나** — 사용자가 global 에 특정 주제 질문을 던지는 것이 관측되면. 또는
리포트 수가 늘어 `rank` 상위 5개가 전체를 대표하지 못하게 되면(현재 라이브 15건).
그때는 임베딩 기반 선정이 유력하다 — BGE-M3 가 이미 있고 리포트는 수십 건 규모라
전량 임베딩 비용이 작다.
