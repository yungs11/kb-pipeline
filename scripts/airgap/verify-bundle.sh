...
check_env() {
  ...
  # ── VL 실패 정책(2026-08-14, Phase 2b-2) ──────────────────────────────────
  # 탈출구를 끈 채 번들을 내보내면 **VL 이 아파도 빈 본문이 조용히 적재**된다.
  # 폴백은 이미 삭제됐으므로 0 은 "이전 동작 복원" 이 아니라 "무증상 빈 문서" 다.
  ...
  if [ -n "$_va" ] && ! echo "$_va" | grep -Eq '^[1-5]$'; then
    echo "  ${YEL}! KBP_VL_PAGE_ATTEMPTS=$_va 는 1~5 범위 밖 — 코드가 클램프한다.${RST}"
    inet=1
  fi
