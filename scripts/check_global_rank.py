...
판정 — 두 축을 따로 낸다
------------------------
**축1(넓은 질문)** 이 global 의 승인된 용도다. 여기서 겹침 0 + rank 순 선정은 정상이다.
축1 이 깨지면(리포트 부재 등) 배포를 막는다.

**축2(특정 주제 질문)** 의 약함은 **알려진 한계로 수용됐다**(2026-08-10 결정 — D38).
그래서 축2 실패는 배포를 막지 않고 **회귀 감시 숫자**로 남긴다. 하나로 합치면
정상 배포가 영구 차단되고, 매번 "보류" 를 뱉는 가드는 곧 아무도 안 돌린다.

D38 을 착수한 뒤에는 `--strict` 로 축2 를 진짜 게이트로 승격한다.

핵심 지표(겹침 0 만 세면 놓친다)
  * **겹침 동점** — 겹침이 잡히긴 하나 변별력 없는 흔한 단어 하나뿐이면 정렬은 `rank`
    가 한다. 실측: 휴가 구어체 4종이 겹침 1(전부 `"휴가"`)로 **같은 리포트 5건**을 골랐다.
  * **서로 다른 질문이 같은 선정 집합** — 선정이 질문을 구분하지 못한다는 직접 증거.
...
def main() -> int:
    ...
    ap.add_argument("--strict", action="store_true",
                    help="축2(특정 주제 질문) 실패도 배포 차단으로 취급 — D38 착수 후 회귀 게이트로 쓴다")
    a = ap.parse_args()
    ...
    # ── 판정 ────────────────────────────────────────────────────────────────
    #
    # **두 축을 따로 판정한다.** 하나로 합치면 못 쓴다 —
    #  * global 의 승인된 용도는 **넓은 질문**이고, 거기서는 겹침 0 + rank 순 선정이
    #    정상이다. 이걸 결함으로 세면 정상 배포를 영구히 막는다.
    #  * 특정 주제 질문의 약함은 **알려진 한계**(D38)로 수용됐다(2026-08-10 결정).
    #    그래서 여기서 실패를 내면 매 실행이 "보류" 가 되고, 가드는 곧 무시된다 —
    #    통과해버리는 가드보다 나쁘지 않지만, 아무도 안 돌리는 가드는 없는 것과 같다.
    #
    # 그래서 축2 는 **회귀 감시**로 쓴다: 배포를 막는 게 아니라 "지금 어디까지 되는가"
    # 를 숫자로 남긴다. 막아야 하는 건 축1 이 깨질 때뿐이다.
    broad_ok = len(broad_sets) >= 1 and bool(reports)
    weak = len(blind) + len(tied)
    distinct = len(spec_sets)

    print("─" * 72)
    print("축1 — 넓은 질문 (승인된 용도)")
    print(f"   선정 집합 종류 {len(broad_sets)}/{len(BROAD_QUESTIONS)}"
          "  (1 = 항상 상위 rank 고정 — 전체 요약 용도로는 의도된 동작)")
    print("축2 — 특정 주제 질문 (알려진 한계 · D38 · 회귀 감시용)")
    print(f"   겹침 0                {len(blind)}/{len(specific)}")
    print(f"   겹침 동점(rank 가 정렬) {len(tied)}/{len(specific)}"
          "   ← 겹침 0 과 실질이 같다")
    print(f"   질문을 구분한 선정      {distinct}/{len(specific)}")
    if misplaced:
        print(f"   기대 리포트가 1위 아님  {len(misplaced)}건")

    if not broad_ok:
        print("\n❌ 배포 보류 — 넓은 질문(승인된 용도)조차 선정이 성립하지 않는다.")
        return 1

    print(f"\n✅ 축1 통과 — 넓은 질문 용도로 배포 가능.")
    print(f"⚠️  축2: {weak}/{len(specific)} 개 특정 주제 질문에서 선정이 질문을 반영하지"
          " 못한다(알려진 한계).")
    print("   이 답변들은 **그럴듯하게** 나온다 — 답변만 봐서는 드러나지 않는다.")
    print("   지금 막고 있는 것: UI 토글이 global 을 '넓은 질문' 용도로 안내하고")
    print("   특정 주제 질문은 일반 검색으로 유도한다.")
    print("   D38 착수 조건: (a) 사용자가 global 에 특정 주제 질문을 던지는 것이 관측되거나")
    print("                  (b) 이 수치가 이전 실행보다 나빠지거나")
    print(f"                  (c) 리포트 수가 늘어 top_k 가 코퍼스의 소수가 될 때"
          f" (지금 {a.top_k}/{len(reports)})")
    if a.strict:
        print("\n--strict — 축2 실패를 배포 차단으로 취급한다.")
        return 1 if (blind or tied or misplaced) else 0
    return 0
