...
## 테스트 문서 (kb-pipeline/test_doc)

| 문서 | 용도 |
|------|------|
| `3-3. 휴가규정(2025.12.05. 개정).pdf` (74KB) | 단건 스모크(가벼움) |
| `신탁업무처리지침2016-02호…15차개정.pdf` (1.7MB) | 표·다페이지 layout 검증 |
| `자산신탁 온톨로지 PoC.pdf` (1.2MB) | 동시처리 반복 부하 |

```bash
./test_ocr_api.sh "../../test_doc/3-3. 휴가규정(2025.12.05. 개정).pdf"
./batch_concurrency_test.py "../../test_doc/3-3. 휴가규정(2025.12.05. 개정).pdf" --repeat 24 -c 8
```
> 파일명에 공백·괄호가 있으니 항상 큰따옴표로 감싼다.
